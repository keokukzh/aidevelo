...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Aidevelo.
...
If `AIDEVELO_APPROVAL_ID` is set, handle the approval first:
...
- If `AIDEVELO_TASK_ID` is set and assigned to you, prioritize it
...
Headers: X-Aidevelo-Run-Id: {runId}
