...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Aidevelo.
...
- Aidevelo must not require extra top-level fields for skill validity
- Aidevelo-specific extensions must live under `metadata.aidevelo` or `metadata.sources`
...
For Aidevelo, the preferred fidelity extension is:
...
- issue/task Aidevelo-only metadata
...
- Aidevelo tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Aidevelo shape:
...
Additional rules for Aidevelo exporters:
...
- omit empty and default-valued Aidevelo fields when possible
...
- Aidevelo exporters should emit `.aidevelo.yaml` as a sidecar by default
...
Aidevelo can map this spec to its runtime model like this:
...
- Aidevelo extension:
  - `.aidevelo.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, and other Aidevelo-specific fidelity

Inline Aidevelo-only metadata that must live inside a shared markdown file should use:

- `metadata.aidevelo`

That keeps the base format broader than Aidevelo.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Aidevelo.

## 20. Cutover

Aidevelo should cut over to this markdown-first package model as the primary portability format.

`aidevelo.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Aidevelo, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
