...
Add first-class **documents** to Aidevelo as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Aidevelo workflow/docs.
...
Update the Aidevelo agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/aidevelo/SKILL.md`
