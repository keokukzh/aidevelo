# ClipHub: Marketplace for Aidevelo Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Aidevelo configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Aidevelo-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Aidevelo company (agents, projects, governance) |
...
A complete Aidevelo company configuration:
...
Single-agent configurations designed to plug into a Aidevelo org:
...
Portable skill files that any Aidevelo agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Aidevelo's skill loading system
...
interface Listing {
  ...
  aideveloVersionMin: string;     // Minimum Aidevelo version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Aidevelo company
...
| `POST` | `/api/listings/:id/install` | Install to Aidevelo company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Aidevelo company (or create new)
  → ClipHub API calls Aidevelo API to:
  ...
  → Redirect to Aidevelo dashboard with new team running
```
...
### 5.3 Creator: Export from Aidevelo → Publish

```
Running Aidevelo company → "Export as Blueprint" (CLI or UI)
  → Aidevelo exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Aidevelo brand blue/purple)
...
| Install flow | One-click deploy to Aidevelo company |
...
{
  "targetCompanyId": "uuid",         // Existing Aidevelo company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `aidevelo-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Aidevelo
- **Backend**: Node.js API (or extend Aidevelo server)
- **Database**: Postgres (can share Aidevelo's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Aidevelo auth (or OAuth2)

### 9.2 Integration with Aidevelo

ClipHub can be:
- **Option A**: A separate app that calls Aidevelo's API to install blueprints
- **Option B**: A built-in section of the Aidevelo UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Aidevelo UI and API.
...
- [ ] Install flow: blueprint → Aidevelo company
...
- [ ] "Export from Aidevelo" CLI command
