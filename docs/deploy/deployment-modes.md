...
Aidevelo supports two runtime modes with different security profiles.
...
```sh
pnpm aideveloai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Aidevelo emits a one-time claim URL at startup:
...
```sh
AIDEVELO_DEPLOYMENT_MODE=authenticated pnpm aideveloai run
```
