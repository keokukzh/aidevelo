...
Build a custom adapter to connect Aidevelo to any agent runtime.
...
2. Build environment with `buildAideveloEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `aideveloai run --watch` using `picocolors`.
...
Make Aidevelo skills discoverable to your agent runtime without writing to the agent's working directory:
