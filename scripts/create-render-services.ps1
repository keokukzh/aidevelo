...
# Create aidevelo-api web service
$apiBody = @{
    name = "aidevelo-api"
    type = "web_service"
    region = "oregon"
...
Write-Host "Creating aidevelo-api..."
...
Write-Host "Status: $($createResp.StatusCode) $($createResp.StatusDescription)"
Write-Host "Content: $($createResp.Content)"

# Also check services list
Write-Host "`nListing services..."
$listResp = Invoke-WebRequest -Uri 'https://api.render.com/v1/services?limit=20' -Headers $headers -Method GET -UseBasicParsing
Write-Host "Services: $($listResp.Content)"
