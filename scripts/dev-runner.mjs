...
const devServerStatusFilePath = path.join(repoRoot, ".aidevelo", "dev-server-status.json");
...
const ignoredRelativePaths = new Set([
  ".aidevelo/dev-server-status.json",
]);
...
const env = {
  ...process.env,
  AIDEVELO_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.AIDEVELO_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
}

if (mode === "watch") {
  env.AIDEVELO_MIGRATION_PROMPT ??= "never";
  env.AIDEVELO_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.AIDEVELO_DEPLOYMENT_MODE = "authenticated";
  env.AIDEVELO_DEPLOYMENT_EXPOSURE = "private";
  env.AIDEVELO_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[aidevelo] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[aidevelo] dev mode: local_trusted (default)");
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@aideveloai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[aidevelo] Command failed with code ${status.code}: pnpm --filter @aideveloai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[aidevelo] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options = {}) {
  ...
  const autoApply = options.autoApply ?? env.AIDEVELO_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[aidevelo] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
...
async function buildPluginSdk() {
  console.log("[aidevelo] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@aideveloai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[aidevelo] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@aideveloai/server", serverScript, ...forwardedArgs],
