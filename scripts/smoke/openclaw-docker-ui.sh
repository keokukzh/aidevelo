...
AIDEVELO_HOST_PORT="${AIDEVELO_HOST_PORT:-3100}"
AIDEVELO_HOST_FROM_CONTAINER="${AIDEVELO_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.aidevelo-openclaw.override.yml"
...
detect_aidevelo_base_url() {
  ...
  for candidate in "$AIDEVELO_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${AIDEVELO_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${AIDEVELO_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
aidevelo_base_url="$(detect_aidevelo_base_url || true)"
...
Aidevelo URL for OpenClaw container:
EOF
  if [[ -n "$aidevelo_base_url" ]]; then
    cat <<EOF
  $aidevelo_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${AIDEVELO_HOST_PORT}
  (Do not use http://127.0.0.1:${AIDEVELO_HOST_PORT} inside the container.)
  If Aidevelo rejects the host, run on host machine:
    pnpm aideveloai allowed-hostname host.docker.internal
  Then restart Aidevelo and re-run this script.
EOF
  fi
  cat <<EOF
