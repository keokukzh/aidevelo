...
COMPANY_ID="${COMPANY_ID:-${AIDEVELO_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-aidevelo-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-aidevelo-openclaw-smoke}"
...
Provide one of:
  AIDEVELO_AUTH_HEADER=\"Bearer <board-token>\"
  AIDEVELO_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Aidevelo OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.aidevelo.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
