...
AIDEVELO_API_URL="${AIDEVELO_API_URL:-http://127.0.0.1:3100}"
API_BASE="${AIDEVELO_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-aidevelo-smoke}"
...
AIDEVELO_API_URL_FOR_OPENCLAW="${AIDEVELO_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${AIDEVELO_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${AIDEVELO_AUTH_HEADER}" )
fi
if [[ -n "${AIDEVELO_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${AIDEVELO_COOKIE}" )
  AIDEVELO_BROWSER_ORIGIN="${AIDEVELO_BROWSER_ORIGIN:-${AIDEVELO_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${AIDEVELO_BROWSER_ORIGIN}" -H "Referer: ${AIDEVELO_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set AIDEVELO_COOKIE or AIDEVELO_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg aideveloApiUrl "$AIDEVELO_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/aidevelo"
  local claimed_file="${workspace_dir}/aidevelo-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/aidevelo-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/aidevelo-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/aidevelo-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/aidevelo"
    assert_status "200"
    {
      echo "AIDEVELO_API_URL=${AIDEVELO_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set AIDEVELO_API_KEY=${AGENT_API_KEY} in your run context before running Aidevelo heartbeat steps."
...
main() {
  ...
  wait_http_ready "${AIDEVELO_API_URL%/}/api/health" 15 || fail "Aidevelo API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "aidevelo health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
