...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Aidevelo data dir to use
  AIDEVELO_HOST=127.0.0.1    Host passed to the onboarded server
  AIDEVELO_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/aidevelo-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/aidevelo-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export AIDEVELO_HOST="${AIDEVELO_HOST:-127.0.0.1}"
export AIDEVELO_PORT="${AIDEVELO_PORT:-3100}"
export AIDEVELO_OPEN_ON_LISTEN="${AIDEVELO_OPEN_ON_LISTEN:-false}"
...
echo "AIDEVELO_HOST: $AIDEVELO_HOST"
echo "AIDEVELO_PORT: $AIDEVELO_PORT"
...
env \
  ...
  HOST="$AIDEVELO_HOST" \
  PORT="$AIDEVELO_PORT" \
  AIDEVELO_OPEN_ON_LISTEN="$AIDEVELO_OPEN_ON_LISTEN" \
  pnpm aideveloai onboard --yes --data-dir "$PC_DATA"
