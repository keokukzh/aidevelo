#!/usr/bin/env bash
#
# Kill all local Aidevelo dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all aidevelo dev processes
...
# Collect PIDs of node processes running from any aidevelo directory.
# Matches paths like /Users/*/aidevelo/... or /Users/*/aidevelo-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/aidevelo(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No Aidevelo dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} Aidevelo dev process(es):"
echo ""
