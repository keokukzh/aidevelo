...
/**
 * Filter .aidevelo.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterAideveloYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://aidevelo.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm aideveloai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Aidevelo](https://aidevelo.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Aidevelo](https://aidevelo.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .aidevelo.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .aidevelo.yaml
    const yamlPath = exportData.aideveloExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterAideveloYaml(exportData.files[yamlPath], checkedFiles);
    }
