...
export function aideveloGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Aidevelo API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "aideveloApiUrl",
                  String(config.aideveloApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "aideveloApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://aidevelo.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "aidevelo"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="aidevelo"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Aidevelo persists a device key during onboarding so pairing approvals
