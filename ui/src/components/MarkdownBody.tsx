...
import { parseProjectMentionHref } from "@aideveloai/shared";
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`aidevelo-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="aidevelo-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("aidevelo-mermaid-status", error && "aidevelo-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="aidevelo-mermaid-source">
...
export function MarkdownBody({ children, className, resolveImageSrc }: MarkdownBodyProps) {
      ...
      if (parsed) {
        ...
        return (
          <a
            ...
            className="aidevelo-project-mention-chip"
  ...
  return (
    <div
      className={cn(
        "aidevelo-markdown prose prose-sm max-w-none break-words overflow-hidden",
