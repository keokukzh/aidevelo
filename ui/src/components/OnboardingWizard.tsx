...
import type { AdapterEnvironmentTestResult } from "@aideveloai/shared";
...
import { CEO_BEHAVIOR_PROFILE } from "@aideveloai/shared";
...
const DEFAULT_TASK_DESCRIPTION = `You are the CEO. Start the company like an operator.

- validate the business opportunity and the strongest customer problem
- define the first concrete revenue path
- identify the first hires or delegation moves needed
- create the next execution plan as specific issues with owners`;

const MANAGED_RUNTIME_CONFIG = {
...
export function OnboardingWizard() {
  ...
  const [proactiveInitiativesEnabled, setProactiveInitiativesEnabled] =
    useState(true);
  const [autonomousHiringEnabled, setAutonomousHiringEnabled] =
    useState(false);
  ...
  useEffect(() => {
    if (!companySkills || !shouldAutoSyncCeoPresetSkills(ceoSkillPreset)) return;
    setSelectedSkillKeys(
      resolveInstalledCeoPresetSkillKeys(companySkills, ceoSkillPreset)
    );
  }, [companySkills, ceoSkillPreset]);
  ...
  function reset() {
    ...
    setProactiveInitiativesEnabled(true);
  ...
  function applySkillPreset(presetId: CeoSkillPresetId) {
    setCeoSkillPreset(presetId);
    if (presetId === "manual") return;
    setSelectedSkillKeys(
      resolveInstalledCeoPresetSkillKeys(companySkills ?? [], presetId)
    );
  }

  function toggleSkill(key: string, checked: boolean) {
    setCeoSkillPreset("manual");
  ...
  async function handleStep1Next() {
    ...
    try {
      const company = await companiesApi.create({
        ...
        runtimeMode: "managed",
        managedRuntimeConfig: MANAGED_RUNTIME_CONFIG
      });
    ...
    try {
      ...
      if (!projectId) {
        const project = await projectsApi.create(createdCompanyId, {
          name: `${agentName.trim() || "CEO"} workspace`,
          description:
            "Default managed workspace project created during onboarding for the CEO and first execution run.",
          status: "in_progress",
          leadAgentId: createdAgentId,
          ...(createdGoalId ? { goalIds: [createdGoalId] } : {})
        });
                ...
                <div id="onboarding-step-2" role="tabpanel" aria-labelledby="onboarding-step-2-heading" className="space-y-5">
                  ...
                  <div className="rounded-md border border-border bg-card/40 p-4 space-y-4">
                    <div className="space-y-1">
                      <p className="text-sm font-medium">Balanced startup CEO</p>
                      <p className="text-xs text-muted-foreground">
                        This CEO is optimized for ideas, growth, revenue,
                        delegation, and budget discipline.
                      </p>
                    </div>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between gap-4 text-sm">
                        <div className="space-y-1">
                          <div>Proactive initiatives</div>
                          <p className="text-xs text-muted-foreground">
                            Let the CEO create one new business-moving initiative
                            when idle.
                          </p>
                        </div>
                        <button
                          type="button"
                          role="switch"
                          aria-checked={proactiveInitiativesEnabled}
                          className={cn(
                          ...
                          onClick={() =>
                            setProactiveInitiativesEnabled((current) => !current)
                          }
                        >
                          <span
                            className={cn(
                      ...
                      <div className="flex items-center justify-between gap-4 text-sm">
                        <div className="space-y-1">
                          <div>Autonomous hiring requests</div>
                          <p className="text-xs text-muted-foreground">
                            Allow the CEO to submit hire requests when a real
                            capacity gap blocks progress.
                          </p>
                        </div>
                        <button
                          type="button"
                          role="switch"
                          aria-checked={autonomousHiringEnabled}
                          className={cn(
                            "relative inline-flex h-5 w-9 items-center rounded-full transition-colors shrink-0",
                            autonomousHiringEnabled ? "bg-green-600" : "bg-muted"
                          )}
                          onClick={() =>
                            setAutonomousHiringEnabled((current) => !current)
                          }
                        >
                          <span
                            className={cn(
                  ...
                  <div className="rounded-md border border-border bg-card/40 p-4 space-y-4">
                    <div className="space-y-1">
                      <p className="text-sm font-medium">CEO skill preset</p>
                      <p className="text-xs text-muted-foreground">
                        Presets only enable skills that are already installed in
                        the company library.
                      </p>
                    </div>
                    <div className="grid gap-2 sm:grid-cols-3">
                      {CEO_SKILL_PRESETS.map((preset) => (
                        <button
                          key={preset.id}
                          type="button"
                          className={cn(
                            "rounded-md border px-3 py-3 text-left transition-colors",
                            ceoSkillPreset === preset.id
                    ...
                    {(companySkills ?? []).length === 0 ? (
                      <p className="text-xs text-muted-foreground">
                        No company skills are installed yet.
                      </p>
                    ) : (
                      <div className="space-y-3">
                    ...
                    <div className="text-xs text-muted-foreground">
                      Selected: {selectedSkillKeys.length} skill
                      {selectedSkillKeys.length === 1 ? "" : "s"}
                  ...
                  {managedOnboarding ? (
                    <div className="rounded-md border border-border bg-card/40 p-4 space-y-3">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-1">
                          <p className="text-sm font-medium">Managed runtime</p>
                          <p className="text-xs text-muted-foreground">
                            Aidevelo configures workflow, routing, and model choice to protect budget by default. The setup is tuned for low usage, lower token consumption, and controlled fallback so you do not overpay while getting started.
                          </p>
                        </div>
                      </div>
                      <div className="grid grid-cols-1 gap-2 text-xs text-muted-foreground sm:grid-cols-2">
                        <div className="rounded-md border border-border/70 bg-background/30 p-3">
                          <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Primary route</p>
                          <p className="mt-1 text-sm text-foreground">Main API</p>
                        </div>
                        <div className="rounded-md border border-border/70 bg-background/30 p-3">
                          <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Fallback route</p>
                          <p className="mt-1 text-sm text-foreground">Fallback API only when necessary</p>
                        </div>
                      </div>
                      <div className="rounded-md border border-border/70 bg-background/30 p-3">
                        <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Workflow setup</p>
                        <p className="mt-1 text-sm text-foreground">Budget-first, token-saving orchestration</p>
                      </div>
                      <div className="rounded-md border border-border/70 bg-background/30 p-3">
                        <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Model selection</p>
                        <p className="mt-1 text-sm text-foreground">Managed automatically to reduce spend</p>
                      </div>
                    </div>
                  ) : (
                    <>
                  ...
                  {!managedOnboarding && isLocalAdapter && (
