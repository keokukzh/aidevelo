 ...
 *
 * Plugin bundles are built with `external: ["@aideveloai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__aideveloPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__aideveloPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__aideveloPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__aideveloPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@aideveloai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@aideveloai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@aideveloai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@aideveloai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@aideveloai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__aideveloPluginBridge__) {
 ...
 * self-register via `window.aideveloPlugins.registerReactComponent()`.
 ...
 * of `@aideveloai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
