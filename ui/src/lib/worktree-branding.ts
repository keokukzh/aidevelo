...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("aidevelo-worktree-enabled") !== "true") return null;

  const name = readMetaContent("aidevelo-worktree-name");
  const color = normalizeHexColor(readMetaContent("aidevelo-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("aidevelo-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
