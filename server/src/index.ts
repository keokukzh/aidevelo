...
import { ensureAgentJwtSecret } from "./ensure-agent-jwt-secret.js";
...
export async function startServer(): Promise<StartedServer> {
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.AIDEVELO_MIGRATION_PROMPT === "never") return false;
    if (process.env.AIDEVELO_MIGRATION_AUTO_APPLY === "true") return true;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set AIDEVELO_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@aidevelo.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.AIDEVELO_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (runningPid) {
      ...
      const configuredAdminConnectionString = `postgres://aidevelo:aidevelo@127.0.0.1:${configuredPort}/postgres`;
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "aidevelo");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://aidevelo:aidevelo@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "aidevelo");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: aidevelo");
    }
  
    const embeddedConnectionString = `postgres://aidevelo:aidevelo@127.0.0.1:${port}/aidevelo`;
  ...
  if (config.deploymentMode === "local_trusted") {
    ...
    await ensureAgentJwtSecret();
  }
  if (config.deploymentMode === "authenticated" && config.authAllowLocalImplicitBoard) {
    await ensureLocalTrustedBoardPrincipal(db as any);
  }
  if (config.deploymentMode === "authenticated") {
    const {
    ...
    if (!betterAuthSecret) {
      throw new Error(
        "authenticated mode requires BETTER_AUTH_SECRET (or AIDEVELO_AGENT_JWT_SECRET) to be set",
      );
    }
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.AIDEVELO_LISTEN_HOST = runtimeListenHost;
  process.env.AIDEVELO_LISTEN_PORT = String(listenPort);
  process.env.AIDEVELO_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.AIDEVELO_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.AIDEVELO_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "Aidevelo server failed to start");
    process.exit(1);
  });
}
