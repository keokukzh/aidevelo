...
import type { WorkspaceOperation } from "@aideveloai/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aidevelo-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "aidevelo@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Aidevelo Test"]);
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".aidevelo", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$AIDEVELO_WORKSPACE_BRANCH\" > .aidevelo-provision-branch",
        "printf '%s\\n' \"$AIDEVELO_WORKSPACE_BASE_CWD\" > .aidevelo-provision-base",
        "printf '%s\\n' \"$AIDEVELO_WORKSPACE_CREATED\" > .aidevelo-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".aidevelo-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".aidevelo-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".aidevelo-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".aidevelo-provision-created"), "utf8")).resolves.toBe("false\n");
  });
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aidevelo-runtime-workspace-"));
  ...
  it("does not leak parent Aidevelo instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aidevelo-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "aideveloConfig: process.env.AIDEVELO_CONFIG ?? null,",
          "aideveloHome: process.env.AIDEVELO_HOME ?? null,",
          "aideveloInstanceId: process.env.AIDEVELO_INSTANCE_ID ?? null,",
    ...
    process.env.AIDEVELO_CONFIG = "/tmp/base-aidevelo-config.json";
    process.env.AIDEVELO_HOME = "/tmp/base-aidevelo-home";
    process.env.AIDEVELO_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/aidevelo";
    ...
    expect(captured.aideveloConfig).toBeNull();
    expect(captured.aideveloHome).toBeNull();
    expect(captured.aideveloInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "aidevelo-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "aidevelo-runtime-sibling-"));
