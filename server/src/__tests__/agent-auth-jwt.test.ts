...
describe("agent local JWT", () => {
  const secretEnv = "AIDEVELO_AGENT_JWT_SECRET";
  const ttlEnv = "AIDEVELO_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "AIDEVELO_AGENT_JWT_ISSUER";
  const audienceEnv = "AIDEVELO_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "aidevelo";
    process.env[audienceEnv] = "aidevelo-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
