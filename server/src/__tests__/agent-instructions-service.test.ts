...
describe("agent instructions service", () => {
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const aideveloHome = await makeTempDir("aidevelo-agent-instructions-home-");
    const externalRoot = await makeTempDir("aidevelo-agent-instructions-external-");
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("aidevelo-agent-instructions-new-external-");
    cleanupDirs.add(aideveloHome);
    cleanupDirs.add(externalRoot);
    process.env.AIDEVELO_HOME = aideveloHome;
    process.env.AIDEVELO_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("aidevelo-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
