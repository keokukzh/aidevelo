...
import { execute } from "@aideveloai/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects aidevelo env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aidevelo-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the aidevelo heartbeat.");
      expect(promptArg).toContain("Aidevelo runtime note:");
      ...
      expect(invocationPrompt).toContain("Aidevelo API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aidevelo-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      await execute({
        ...
        config: {
          command: commandPath,
          cwd: workspace,
          env: { AIDEVELO_TEST_CAPTURE_PATH: capturePath },
        },
        context: {},
        authToken: "t",
        onLog: async () => {},
