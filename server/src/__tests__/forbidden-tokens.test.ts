...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "aidevelo", LOGNAME: "aidevelo", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "aidevelo" }),
      },
    );

    expect(tokens).toEqual(["aidevelo", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "aidevelo" }, {
        userInfo: () => ({ username: "aidevelo" }),
      });

      expect(tokens).toEqual(["aidevelo", "custom-token"]);
    } finally {
