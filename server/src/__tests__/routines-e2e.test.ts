...
vi.mock("../services/index.js", async () => {
  ...
  const { heartbeatRuns, issues } = await import("@aideveloai/db");
...
async function startTempDatabase() {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "aidevelo-routines-e2e-"));
  ...
  const connectionString = `postgres://aidevelo:aidevelo@127.0.0.1:${port}/aidevelo`;
