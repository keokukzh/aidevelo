...
import type { CompanyPortabilityFileEntry } from "@aideveloai/shared";
...
describe("company portability", () => {
  ...
  it("exports referenced skills as stubs by default with sanitized Aidevelo extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Aidevelo"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${aideveloKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/aideveloai/aidevelo/aidevelo/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/aideveloai/aidevelo/aidevelo/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/aideveloai/aidevelo/aidevelo/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".aidevelo.yaml"]);
    expect(extension).toContain('schema: "aidevelo/v1"');
    ...
    expect(extension).not.toContain("aideveloSkillSync");
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/aideveloai/aidevelo/aidevelo/SKILL.md"])).toContain("# Aidevelo");
    expect(asTextFile(exported.files["skills/aideveloai/aidevelo/aidevelo/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/aideveloai/aidevelo/aidevelo/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/aideveloai/aidevelo/aidevelo/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/aideveloai/aidevelo/aidevelo/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .aidevelo.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".aidevelo.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-aidevelo",
        companyId: "company-1",
        key: "aideveloai/aidevelo/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/aideveloai/aidevelo/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
    ...
    expect(asTextFile(exported.files["skills/aideveloai/aidevelo/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/aideveloai/aidevelo/release-changelog/SKILL.md"])).toContain("aideveloai/aidevelo/release-changelog");
  });
  ...
  it("reads env inputs back from .aidevelo.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    const preview = await portability.previewImport({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Aidevelo",
      },
      agents: "all",
  ...
  it("imports a vendor-neutral package without .aidevelo.yaml", async () => {
    const portability = companyPortabilityService({} as any);

    companySvc.create.mockResolvedValue({
      id: "company-imported",
      name: "Imported Aidevelo",
    });
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Aidevelo");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".aidevelo.yaml"] = `${exported.files[".aidevelo.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
