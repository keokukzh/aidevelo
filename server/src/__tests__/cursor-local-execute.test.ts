...
import { execute } from "@aideveloai/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects aidevelo env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aidevelo-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the aidevelo heartbeat.");
      ...
      expect(capture.prompt).toContain("Aidevelo runtime note:");
      expect(capture.prompt).toContain("AIDEVELO_API_KEY");
      expect(invocationPrompt).toContain("Aidevelo runtime note:");
      expect(invocationPrompt).toContain("AIDEVELO_API_URL");
    } finally {
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aidevelo-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "aidevelo-cursor-execute-runtime-skill-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    ...
    const aideveloDir = await createSkillDir(runtimeSkillsRoot, "aidevelo");
