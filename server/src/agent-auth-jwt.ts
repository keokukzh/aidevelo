...
function jwtConfig() {
  const secret = process.env.AIDEVELO_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.AIDEVELO_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.AIDEVELO_AGENT_JWT_ISSUER ?? "aidevelo",
    audience: process.env.AIDEVELO_AGENT_JWT_AUDIENCE ?? "aidevelo-api",
  };
}
