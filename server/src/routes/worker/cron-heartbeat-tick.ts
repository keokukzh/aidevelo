// Fallback/healthkick endpoint for environments where worker liveness is uncertain.
// This route does not run scheduling logic directly; it only enqueues a heartbeat tick
// so the worker queue path remains the single source of truth.
...
export function cronHeartbeatTickRoutes(_db?: Db) {
  ...
  router.post("/heartbeat-tick", async (req, res) => {
    ...
    try {
      const enqueue = await jobQueue.enqueueHeartbeatTick(
        new Date().toISOString(),
        { source: "cron_heartbeat_healthkick" },
      );
