...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "aidevelo" &&
    normalized !== "aidevelo-create-agent" &&
    normalized !== "aidevelo-create-plugin" &&
...
/** Resolve the Aidevelo repo skills directory (built-in / managed skills). */
function resolveAideveloSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const aideveloSkillsDir = resolveAideveloSkillsDir();

  // Build set of Aidevelo-managed skill names
  const aideveloSkillNames = new Set<string>();
  if (aideveloSkillsDir) {
    try {
      for (const entry of fs.readdirSync(aideveloSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) aideveloSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isAideveloManaged: aideveloSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.aideveloApiUrl)) {
    const legacyAideveloApiUrl = nonEmptyTrimmedString(input.aideveloApiUrl);
    if (legacyAideveloApiUrl) merged.aideveloApiUrl = legacyAideveloApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    aideveloApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.aideveloApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawAideveloApiUrl =
    typeof defaults.aideveloApiUrl === "string"
      ? defaults.aideveloApiUrl.trim()
      : "";
  if (rawAideveloApiUrl) {
    try {
      const parsedAideveloApiUrl = new URL(rawAideveloApiUrl);
      if (
        parsedAideveloApiUrl.protocol !== "http:" &&
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/aidevelo";
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    The board approves the join request in Aidevelo before key claim is allowed.
    ...
    Then set AIDEVELO_API_KEY and AIDEVELO_API_URL from the saved token field for every heartbeat run.
    ...
    Be sure to prepend your AIDEVELO_API_URL to the top of your skill and note the path to your AIDEVELO_API_URL
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Aidevelo base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.aideveloApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm aideveloai allowed-hostname <host>
      - then restart Aidevelo and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@aidevelo.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "aidevelo", path: "/api/skills/aidevelo" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "aidevelo-create-agent",
          path: "/api/skills/aidevelo-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.aideveloApiUrl &&
          !persistedDefaults.aideveloApiUrl
        ) {
