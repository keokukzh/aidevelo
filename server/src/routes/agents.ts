...
import { agents as agentsTable, companies, heartbeatRuns } from "@aideveloai/db";
...
import { runClaudeLogin } from "@aideveloai/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@aideveloai/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@aideveloai/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@aideveloai/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@aideveloai/adapter-opencode-local/server";
import {
  loadDefaultAgentInstructionsBundle,
  resolveDefaultAgentInstructionsBundleRole,
} from "../services/default-agent-instructions.js";
import { provisionAgentHomeForRole } from "../services/agent-home-provisioning.js";
import { resolveRequestedAgentAdapter } from "../managed-runtime.js";
...
export function agentRoutes(db: Db) {
  ...
  const DEFAULT_INSTRUCTIONS_PATH_KEYS: Record<string, string> = {
    ...
    managed_service: "instructionsFilePath",
  };
  ...
  const strictSecretsMode = process.env.AIDEVELO_SECRETS_STRICT_MODE === "true";
  ...
  async function assertAdapterConfigConstraints(
    ...
    if (adapterType === "managed_service") return;
  ...
  async function buildRuntimeSkillConfig(
    ...
    return {
      ...config,
      aideveloRuntimeSkills: runtimeSkillEntries,
    };
  }
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeAideveloSkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }

  function withNewCeoBehaviorDefaults(role: string | null | undefined, adapterConfig: Record<string, unknown>) {
    if (role !== "ceo") return adapterConfig;
    const storedBehavior = asRecord(adapterConfig.ceoBehavior) ?? {};
    return setCeoBehavior(adapterConfig, storedBehavior, "new");
  }

  function normalizeExplicitCeoBehaviorPatch(
    role: string | null | undefined,
    adapterConfig: Record<string, unknown>,
    previousAdapterConfig: Record<string, unknown>,
  ) {
    if (role !== "ceo") return adapterConfig;
    if (!Object.prototype.hasOwnProperty.call(adapterConfig, "ceoBehavior")) return adapterConfig;
    const storedBehavior = asRecord(adapterConfig.ceoBehavior) ?? {};
    const defaultMode = hasStoredCeoBehavior(previousAdapterConfig) ? "existing" : "existing";
    return setCeoBehavior(adapterConfig, storedBehavior, defaultMode);
  }

  async function persistSyncedCeoInstructions<T extends {
    id: string;
    companyId: string;
    name: string;
    role: string;
    ...
    const syncResult = await syncManagedCeoInstructions(agent);
    const nextAdapterConfig = syncResult.adapterConfig;
    const unchanged =
      JSON.stringify(asRecord(agent.adapterConfig) ?? {}) === JSON.stringify(nextAdapterConfig);
    if (unchanged) {
      return syncResult.agent as T;
    }
    ...
    const updated = await svc.update(
      ...
      options?.recordRevision ? { recordRevision: options.recordRevision } : undefined,
    );
    return ((updated as T | null) ?? { ...syncResult.agent, adapterConfig: normalizedAdapterConfig }) as T;
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readAideveloSkillSyncPreference(
  ...
  router.post("/companies/:companyId/agent-hires", validate(createAgentHireSchema), async (req, res) => {
    ...
    const requestedAdapterConfig = withNewCeoBehaviorDefaults(
      hireInput.role,
      ((hireInput.adapterConfig ?? {}) as Record<string, unknown>),
    );
    const resolvedAdapter = resolveRequestedAgentAdapter(
      company,
    ...
    const desiredSkillAssignment = await resolveDesiredSkillAssignment(
      companyId,
      resolvedAdapter.adapterType ?? "process",
      resolvedAdapter.adapterConfig,
    ...
    const materializedAgent = await materializeDefaultInstructionsBundleForNewAgent(createdAgent);
    const agent = await persistSyncedCeoInstructions(materializedAgent);
    await provisionAgentHomeForRole(agent.id, agent.role);
  ...
  router.patch("/agents/:id/instructions-bundle", validate(updateAgentInstructionsBundleSchema), async (req, res) => {
    ...
    const currentAgent = (await svc.getById(id)) ?? { ...existing, adapterConfig: normalizedAdapterConfig };
    const syncedAgent = await persistSyncedCeoInstructions(currentAgent, {
      recordRevision: {
        createdByAgentId: actor.agentId,
        createdByUserId: actor.actorType === "user" ? actor.actorId : null,
        source: "instructions_bundle_patch",
    ...
    res.json(await instructions.getBundle(syncedAgent));
  });
  ...
  router.put("/agents/:id/instructions-bundle/file", validate(upsertAgentInstructionsFileSchema), async (req, res) => {
    ...
    res.json(await instructions.readFile(syncedAgent, req.body.path));
  });
  ...
  router.patch("/agents/:id", validate(updateAgentSchema), async (req, res) => {
    ...
    const becameCeo = existing.role !== "ceo" && requestedRole === "ceo";
    if (Object.prototype.hasOwnProperty.call(patchData, "adapterConfig")) {
      ...
      patchData.adapterConfig = normalizeExplicitCeoBehaviorPatch(
        requestedRole,
        adapterConfig,
    ...
    const explicitCeoBehaviorPatch =
      Object.prototype.hasOwnProperty.call(patchData, "adapterConfig")
      && Object.prototype.hasOwnProperty.call(asRecord(patchData.adapterConfig) ?? {}, "ceoBehavior");
    if (touchesAdapterConfiguration) {
      ...
      patchData.adapterType = resolvedAdapter.adapterType;
      patchData.adapterConfig = syncInstructionsBundleConfigFromFilePath(existing, normalizedEffectiveAdapterConfig);
    }
    if (touchesAdapterConfiguration && patchData.adapterType === "opencode_local") {
      const effectiveAdapterConfig = asRecord(patchData.adapterConfig) ?? {};
      await assertAdapterConfigConstraints(
        existing.companyId,
        typeof patchData.adapterType === "string" ? patchData.adapterType : requestedAdapterType,
    ...
    const syncedAgent = (becameCeo || explicitCeoBehaviorPatch)
      ? await persistSyncedCeoInstructions(agent, {
          recordRevision: {
            createdByAgentId: actor.agentId,
            createdByUserId: actor.actorType === "user" ? actor.actorId : null,
            source: "patch",
    ...
    await logActivity(db, {
      companyId: syncedAgent.companyId,
      actorType: actor.actorType,
      actorId: actor.actorId,
      agentId: actor.agentId,
      runId: actor.runId,
      ...
      entityType: "agent",
      entityId: syncedAgent.id,
      details: summarizeAgentUpdateDetails(patchData),
    });

    res.json(syncedAgent);
  });
  ...
  router.post("/agents/:id/claude-login", async (req, res) => {
    ...
    assertCompanyAccess(req, agent.companyId);
    if (!CLAUDE_LOGIN_COMPATIBLE_ADAPTER_TYPE_SET.has(agent.adapterType)) {
      res.status(400).json({
