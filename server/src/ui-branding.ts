const FAVICON_BLOCK_START = "<!-- AIDEVELO_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- AIDEVELO_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- AIDEVELO_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- AIDEVELO_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.AIDEVELO_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.AIDEVELO_WORKTREE_NAME) ?? nonEmpty(env.AIDEVELO_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.AIDEVELO_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="aidevelo-worktree-enabled" content="true" />',
    `<meta name="aidevelo-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="aidevelo-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="aidevelo-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
