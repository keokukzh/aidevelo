 ...
 *    (`~/.aidevelo/plugins/`) and `node_modules` for packages matching
 *    the `aidevelo-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Aidevelo plugins.
 * Packages matching this pattern are considered Aidevelo plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "aidevelo-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: AideveloPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.aidevelo/plugins/ local directory
  | "npm"               // npm packages matching aidevelo-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: AideveloPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the aidevelo-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "aidevelo-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: AideveloPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Aidevelo plugins installed as npm packages in the current
   * Node.js environment matching the "aidevelo-plugin-*" naming convention.
   *
   ...
   * the "aideveloPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Aidevelo plugin.
   * Throws if the package is a Aidevelo plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<AideveloPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: AideveloPluginManifestV1;
    newManifest: AideveloPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Aidevelo plugin naming convention.
 * Accepts both the "aidevelo-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @aideveloai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "aideveloPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const aideveloPlugin = pkgJson["aideveloPlugin"];
  if (
    aideveloPlugin !== null &&
    typeof aideveloPlugin === "object" &&
    !Array.isArray(aideveloPlugin)
  ) {
    const manifestRelPath = (aideveloPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: AideveloPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumAideveloVersion;
}
 ...
 *   packageName: "aidevelo-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: AideveloPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as AideveloPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Aidevelo plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<AideveloPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Aidevelo plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasAideveloPlugin = "aideveloPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasAideveloPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<AideveloPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: AideveloPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @aideveloai/shared exports). Run those workers through
