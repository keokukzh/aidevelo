...
import type { BillingType } from "@aideveloai/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const DEFERRED_WAKE_CONTEXT_KEY = "_aideveloWakeContext";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__aidevelo_repo_only__";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    context.aideveloWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.aideveloRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.aideveloRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.aideveloSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.aideveloSessionRotationReason = sessionCompaction.reason;
      context.aideveloPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.aideveloSessionHandoffMarkdown;
      delete context.aideveloSessionRotationReason;
      delete context.aideveloPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.aideveloRuntimeServices = runtimeServices;
        context.aideveloRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[aidevelo] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.aideveloRuntimeServices = combinedRuntimeServices;
        context.aideveloRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[aidevelo] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
