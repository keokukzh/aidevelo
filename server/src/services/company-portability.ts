...
import {
  readAideveloSkillSyncPreference,
  writeAideveloSkillSyncPreference,
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".aidevelo.yaml" || normalized === ".aidevelo.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const aidevelo = isPlainRecord(metadata?.aidevelo) ? metadata?.aidevelo as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.aideveloSkillKey)
    ?? asString(aidevelo?.skillKey)
    ?? asString(aidevelo?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "aidevelo_bundled") {
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "aidevelo" },
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[aideveloExtensionPath];
  if (selectedFiles.has(aideveloExtensionPath) && typeof extensionEntry === "string") {
    filtered[aideveloExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findAideveloExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".aidevelo.yaml"] === "string") return ".aidevelo.yaml";
  if (typeof files[".aidevelo.yml"] === "string") return ".aidevelo.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.aidevelo.yaml") || entry.endsWith("/.aidevelo.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "aidevelo_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "aideveloai/aidevelo",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/aideveloai/aidevelo/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "aidevelo_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.aideveloSkillKey = skill.key;
  metadata.aidevelo = {
    ...(isPlainRecord(metadata.aidevelo) ? metadata.aidevelo : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const aideveloExtensionPath = findAideveloExtensionPath(normalizedFiles);
  const aideveloExtension = aideveloExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, aideveloExtensionPath) ?? "")
    : {};
  const aideveloCompany = isPlainRecord(aideveloExtension.company) ? aideveloExtension.company : {};
  const aideveloAgents = isPlainRecord(aideveloExtension.agents) ? aideveloExtension.agents : {};
  const aideveloProjects = isPlainRecord(aideveloExtension.projects) ? aideveloExtension.projects : {};
  const aideveloTasks = isPlainRecord(aideveloExtension.tasks) ? aideveloExtension.tasks : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(aideveloCompany.brandColor),
      logoPath: asString(aideveloCompany.logoPath) ?? asString(aideveloCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof aideveloCompany.requireBoardApprovalForNewAgents === "boolean"
          ? aideveloCompany.requireBoardApprovalForNewAgents
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(aideveloAgents[slug]) ? aideveloAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(aideveloProjects[slug]) ? aideveloProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(aideveloTasks[slug]) ? aideveloTasks[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".aidevelo.yaml" ||
          relative === ".aidevelo.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const aideveloAgentsOut: Record<string, Record<string, unknown>> = {};
    const aideveloProjectsOut: Record<string, Record<string, unknown>> = {};
    const aideveloTasksOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readAideveloSkillSyncPreference(
        ...
        aideveloAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      aideveloProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      aideveloTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const aideveloExtensionPath = ".aidevelo.yaml";
    const aideveloAgents = Object.fromEntries(
      Object.entries(aideveloAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const aideveloProjects = Object.fromEntries(
      Object.entries(aideveloProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const aideveloTasks = Object.fromEntries(
      Object.entries(aideveloTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[aideveloExtensionPath] = buildYamlFile(
      {
        schema: "aidevelo/v1",
        ...
        agents: Object.keys(aideveloAgents).length > 0 ? aideveloAgents : undefined,
        projects: Object.keys(aideveloProjects).length > 0 ? aideveloProjects : undefined,
        tasks: Object.keys(aideveloTasks).length > 0 ? aideveloTasks : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, aideveloExtensionPath);
  ...
  async function buildPreview(
    ...
    if (include.issues) {
      for (const issue of manifest.issues) {
        ...
        if (issue.recurrence) {
          warnings.push(`Task ${issue.slug} has recurrence metadata; Aidevelo will import it as a one-time issue for now.`);
        }
      }
    }
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeAideveloSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
