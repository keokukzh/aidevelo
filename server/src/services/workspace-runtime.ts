...
function buildWorkspaceCommandEnv(input: {
  ...
  env.AIDEVELO_WORKSPACE_CWD = input.worktreePath;
  env.AIDEVELO_WORKSPACE_PATH = input.worktreePath;
  env.AIDEVELO_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.AIDEVELO_WORKSPACE_BRANCH = input.branchName;
  env.AIDEVELO_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.AIDEVELO_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.AIDEVELO_WORKSPACE_SOURCE = input.base.source;
  env.AIDEVELO_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.AIDEVELO_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.AIDEVELO_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.AIDEVELO_PROJECT_ID = input.base.projectId ?? "";
  env.AIDEVELO_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.AIDEVELO_AGENT_ID = input.agent.id;
  env.AIDEVELO_AGENT_NAME = input.agent.name;
  env.AIDEVELO_COMPANY_ID = input.agent.companyId;
  env.AIDEVELO_ISSUE_ID = input.issue?.id ?? "";
  env.AIDEVELO_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.AIDEVELO_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.AIDEVELO_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.AIDEVELO_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.AIDEVELO_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.AIDEVELO_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.AIDEVELO_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.AIDEVELO_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.AIDEVELO_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.AIDEVELO_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.AIDEVELO_PROJECT_ID = input.workspace.projectId ?? "";
  env.AIDEVELO_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.AIDEVELO_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".aidevelo", "worktrees");
