...
import type { AideveloPluginManifestV1 } from "@aideveloai/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __aidevelo_exports: module.exports,
      __aidevelo_module: module,
      __aidevelo_require: requireInSandbox,
      __aidevelo_filename: realPath,
      __aidevelo_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__aidevelo_exports, __aidevelo_module, __aidevelo_require, __aidevelo_filename, __aidevelo_dirname)`;
