...
import { companySkills } from "@aideveloai/db";
import { readAideveloSkillSyncPreference, writeAideveloSkillSyncPreference } from "@aideveloai/adapter-utils/server-utils";
import type { AideveloSkillEntry } from "@aideveloai/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@aideveloai/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveAideveloInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("aideveloai/aidevelo/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveAideveloInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "aidevelo_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Aidevelo skills are read-only.",
      sourceLabel: "Aidevelo bundled",
      sourceBadge: "aidevelo",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Aidevelo workspace",
        sourceBadge: "aidevelo",
...
export function companySkillService(db: Db) {
    ...
    for (const skill of skills) {
      ...
      out.push({
        ...
        requiredReason: required
          ? "Bundled Aidevelo skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "aidevelo_bundled"
        && incomingKind === "github"
        && incomingOwner === "aideveloai"
        && incomingRepo === "aidevelo"
      ) {
        out.push(existing);
        continue;
      }
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeAideveloSkillSyncPreference(config, filtered),
        });
      }
    }
