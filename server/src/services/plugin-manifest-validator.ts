 ...
 */
import { pluginManifestV1Schema } from "@aideveloai/shared";
import type { AideveloPluginManifestV1 } from "@aideveloai/shared";
import { PLUGIN_API_VERSION } from "@aideveloai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): AideveloPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as AideveloPluginManifestV1,
        };
      }
