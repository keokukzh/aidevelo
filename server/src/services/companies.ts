...
export function companyService(db: Db) {
  ...
  const companySelection = {
    ...
    runtimeMode: companies.runtimeMode,
    managedRuntimeConfig: companies.managedRuntimeConfig,
  ...
  return {
    ...
    remove: (id: string) =>
      db.transaction(async (tx) => {
        ...
        // activity_log.run_id can reference heartbeat_runs.id, so remove activity logs first.
        await tx.delete(activityLog).where(eq(activityLog.companyId, id));
        await tx.delete(financeEvents).where(eq(financeEvents.companyId, id));
        await tx.delete(costEvents).where(eq(costEvents.companyId, id));
        // cost_events + finance_events both reference heartbeat_runs, so delete them first.
        await tx.delete(heartbeatRuns).where(eq(heartbeatRuns.companyId, id));
        // heartbeat_runs.wakeup_request_id can reference agent_wakeup_requests.id.
