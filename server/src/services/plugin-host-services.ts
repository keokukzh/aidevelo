...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@aideveloai/db";
 ...
 *
 * This implementation delegates to the core Aidevelo domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@aideveloai/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
