...
import type { AideveloPlugin } from "./define-plugin.js";
import type {
 ...
 * import { startWorkerRpcHost } from "@aideveloai/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  let manifest: AideveloPluginManifestV1 | null = null;
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@aideveloai/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
