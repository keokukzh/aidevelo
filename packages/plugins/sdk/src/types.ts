/**
 * Core types for the Aidevelo plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@aideveloai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @aideveloai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Aidevelo object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Aidevelo UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Aidevelo domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Aidevelo domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Aidevelo secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Aidevelo secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@aideveloai/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@aideveloai/plugin-sdk";
 *
