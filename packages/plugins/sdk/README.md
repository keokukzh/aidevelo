# `@aideveloai/plugin-sdk`

Official TypeScript SDK for Aidevelo plugin authors.

- **Worker SDK:** `@aideveloai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@aideveloai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@aideveloai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@aideveloai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@aideveloai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@aideveloai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@aideveloai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@aideveloai/plugin-sdk/ui/hooks` | Hooks only |
| `@aideveloai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@aideveloai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@aideveloai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@aideveloai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@aideveloai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@aideveloai/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @aideveloai/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main Aidevelo app. They can call ordinary Aidevelo HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Aidevelo instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@aideveloai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@aideveloai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Aidevelo information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@aideveloai/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@aideveloai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@aideveloai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@aideveloai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@aideveloai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@aideveloai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@aideveloai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@aideveloai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@aideveloai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@aideveloai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@aideveloai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@aideveloai/plugin-sdk/bundlers";
...
```bash
aidevelo-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@aideveloai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__aidevelo__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__aidevelo__/events` streams `reload` SSE events on UI build changes
