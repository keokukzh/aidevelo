...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".aidevelo-sdk");
...
/**
 * Generate a complete Aidevelo plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Aidevelo plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "aidevelo-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["aidevelo", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@aideveloai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@aideveloai/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import type { AideveloPluginManifestV1 } from "@aideveloai/plugin-sdk";

const manifest: AideveloPluginManifestV1 = {
  id: ${quote(manifestId)},
  apiVersion: 1,
  version: "0.1.0",
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@aideveloai/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@aideveloai/plugin-sdk\` and \`@aideveloai/shared\` from a local Aidevelo checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.aidevelo-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-aidevelo-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <aidevelo-sdk-path>]");
    process.exit(1);
  }
