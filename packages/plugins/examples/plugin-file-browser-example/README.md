...
Example Aidevelo plugin that demonstrates:
...
```bash
pnpm --filter @aideveloai/plugin-file-browser-example build
pnpm aideveloai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm aideveloai plugin uninstall aidevelo-file-browser-example --force
```
...
- Optional: use `aidevelo-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
