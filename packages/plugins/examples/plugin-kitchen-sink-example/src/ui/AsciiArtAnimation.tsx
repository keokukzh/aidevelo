...
type AideveloSprite = (typeof AIDEVELO_SPRITES)[number];
...
function spriteSize(sprite: AideveloSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = AIDEVELO_SPRITES[(baseCol + baseRow) % AIDEVELO_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = AIDEVELO_SPRITES[Math.floor(Math.random() * AIDEVELO_SPRITES.length)]!;
