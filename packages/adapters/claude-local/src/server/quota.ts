...
import type { ProviderQuotaResult, QuotaWindow } from "@aideveloai/adapter-utils";
...
export async function getQuotaWindows(): Promise<ProviderQuotaResult> {
  ...
  if (
    (hasNonEmptyProcessEnv("ANTHROPIC_API_KEY") || hasNonEmptyProcessEnv("ANTHROPIC_AUTH_TOKEN")) &&
    !authDescription
  ) {
    return {
      provider: "anthropic",
      ...
      error:
        errors[0]
        ?? "ANTHROPIC_API_KEY or ANTHROPIC_AUTH_TOKEN is set and no local Claude subscription session is available for quota polling",
      windows: [],
    };
  }
    ...
    return {
      ...
      error:
        errors.length > 0
          ...
          : `${authDescription}, but Aidevelo could not load subscription quota data`,
      windows: [],
    };
  }
