...
Use when:
- You want Aidevelo to run Cursor Agent CLI locally as the agent runtime
...
Don't use when:
- You need webhook-style external invocation (use aidevelo_gateway or http)
...
- Aidevelo auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$aidevelo" and related skills on local runs.
- Aidevelo auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
