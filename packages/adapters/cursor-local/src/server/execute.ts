...
function renderAideveloEnvNote(env: Record<string, string>): string {
  const aideveloKeys = Object.keys(env)
    .filter((key) => key.startsWith("AIDEVELO_"))
    .sort();
  if (aideveloKeys.length === 0) return "";
  return [
    "Aidevelo runtime note:",
    `The following AIDEVELO_* environment variables are available in this run: ${aideveloKeys.join(", ")}`,
...
export async function ensureCursorSkillsInjected(
  ...
  const skillsEntries = options.skillsEntries
    ?? (options.skillsDir
      ...
      : await readAideveloRuntimeSkillEntries({}, __moduleDir));
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[aidevelo] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[aidevelo] Removed maintainer-only Cursor skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of skillsEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[aidevelo] ${result === "repaired" ? "Repaired" : "Injected"} Cursor skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[aidevelo] Failed to inject Cursor skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const cursorSkillEntries = await readAideveloRuntimeSkillEntries(config, __moduleDir);
  const desiredCursorSkillNames = resolveAideveloDesiredSkillNames(config, cursorSkillEntries);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[aidevelo] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const aideveloEnvNote = renderAideveloEnvNote(env);
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: aideveloEnvNote.length,
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[aidevelo] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
