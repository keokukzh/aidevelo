...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "AIDEVELO_API_URL") || !hasNonEmptyEnvValue(env, "AIDEVELO_API_KEY")) return "";
  return [
    "Aidevelo API access note:",
    "Use run_shell_command with curl to make Aidevelo API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $AIDEVELO_API_KEY\\" \\"$AIDEVELO_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $AIDEVELO_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Aidevelo-Run-Id: $AIDEVELO_RUN_ID\\" -d '{...}' \\"$AIDEVELO_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
/**
 * Inject Aidevelo skills directly into `~/.gemini/skills/` via symlinks.
...
async function ensureGeminiSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[aidevelo] Failed to prepare Gemini skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[aidevelo] Removed maintainer-only Gemini skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[aidevelo] ${result === "repaired" ? "Repaired" : "Linked"} Gemini skill: ${entry.key}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[aidevelo] Failed to link Gemini skill "${entry.key}": ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const geminiSkillEntries = await readAideveloRuntimeSkillEntries(config, __moduleDir);
  const desiredGeminiSkillNames = resolveAideveloDesiredSkillNames(config, geminiSkillEntries);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[aidevelo] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: aideveloEnvNote.length + apiAccessNote.length,
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[aidevelo] Gemini resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
