...
Use when:
- You want Aidevelo to run Pi (the AI coding agent) locally as the agent runtime
...
- Aidevelo requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/aidevelos/ and resumed with --session.
