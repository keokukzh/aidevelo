...
const AIDEVELO_SESSIONS_DIR = path.join(os.homedir(), ".pi", "aidevelos");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[aidevelo] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[aidevelo] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[aidevelo] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(AIDEVELO_SESSIONS_DIR, { recursive: true });
  return AIDEVELO_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(AIDEVELO_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // Inject skills
  const piSkillEntries = await readAideveloRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveAideveloDesiredSkillNames(config, piSkillEntries);
  ...
  if (workspaceCwd) env.AIDEVELO_WORKSPACE_CWD = workspaceCwd;
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[aidevelo] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  const buildArgs = (sessionFile: string): string[] => {
    ...
    // Add Aidevelo skills directory so Pi can load the aidevelo skill
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[aidevelo] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
