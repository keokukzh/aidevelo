...
import type { AdapterModel } from "@aideveloai/adapter-utils";
import { asString, runChildProcess } from "@aideveloai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.AIDEVELO_PI_COMMAND === "string" &&
    process.env.AIDEVELO_PI_COMMAND.trim().length > 0
      ? process.env.AIDEVELO_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["AIDEVELO_", "npm_", "NPM_"] as const;
