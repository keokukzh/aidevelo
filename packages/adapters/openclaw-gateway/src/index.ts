...
- workspaceRuntime (object, optional): desired runtime service intents; Aidevelo forwards these in a standardized aidevelo.workspaceRuntime block for remote execution environments
- timeoutSec (number, optional): adapter timeout in seconds (default 120)
- waitTimeoutMs (number, optional): agent.wait timeout override (default timeoutSec * 1000)
- autoPairOnFirstConnect (boolean, optional): on first "pairing required", attempt device.pair.list/device.pair.approve via shared auth, then retry once (default true)
- aideveloApiUrl (string, optional): absolute Aidevelo base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default aidevelo)

Standard outbound payload additions:
- aidevelo (object): standardized Aidevelo context added to every gateway agent request
- aidevelo.workspace (object, optional): resolved execution workspace for this run
- aidevelo.workspaces (array, optional): additional workspace hints Aidevelo exposed to the run
- aidevelo.workspaceRuntime (object, optional): normalized runtime service intent config for the workspace
