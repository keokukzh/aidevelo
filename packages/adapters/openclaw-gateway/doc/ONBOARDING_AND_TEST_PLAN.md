...
## Scope
This plan is now **gateway-only**. Aidevelo supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Aidevelo skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Aidevelo task.
...
```bash
pnpm --filter @aideveloai/server test:run -- openclaw-gateway
pnpm --filter @aideveloai/server typecheck
pnpm --filter @aideveloai/ui typecheck
pnpm --filter aideveloai typecheck
```
