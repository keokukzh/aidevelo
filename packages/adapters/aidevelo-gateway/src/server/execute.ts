...
} from "@aideveloai/adapter-utils";
import { asNumber, asString, buildAideveloEnv, parseObject } from "@aideveloai/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "aidevelo";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "aidevelo";
  if (input.strategy === "run") return `aidevelo:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `aidevelo:issue:${input.issueId}`;
  return fallback;
}
...
function resolveAideveloApiUrlOverride(value: unknown): string | null {
...
function buildAideveloEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const aideveloApiUrlOverride = resolveAideveloApiUrlOverride(ctx.config.aideveloApiUrl);
  const aideveloEnv: Record<string, string> = {
    ...buildAideveloEnv(ctx.agent),
    AIDEVELO_RUN_ID: ctx.runId,
  };

  if (aideveloApiUrlOverride) {
    aideveloEnv.AIDEVELO_API_URL = aideveloApiUrlOverride;
  }
  if (wakePayload.taskId) aideveloEnv.AIDEVELO_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) aideveloEnv.AIDEVELO_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) aideveloEnv.AIDEVELO_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) aideveloEnv.AIDEVELO_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) aideveloEnv.AIDEVELO_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    aideveloEnv.AIDEVELO_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(payload: WakePayload, aideveloEnv: Record<string, string>): string {
  ...
  const orderedKeys = [
    ...
    "AIDEVELO_API_URL",
    "AIDEVELO_TASK_ID",
    "AIDEVELO_WAKE_REASON",
    "AIDEVELO_WAKE_COMMENT_ID",
    "AIDEVELO_APPROVAL_ID",
    "AIDEVELO_APPROVAL_STATUS",
    "AIDEVELO_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = aideveloEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = aideveloEnv.AIDEVELO_API_URL ?? "<set AIDEVELO_API_URL>";

  const lines = [
    "Aidevelo wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `AIDEVELO_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load AIDEVELO_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $AIDEVELO_API_KEY on every API call.",
    "- Use X-Aidevelo-Run-Id: $AIDEVELO_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: AIDEVELO_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$AIDEVELO_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$AIDEVELO_COMPANY_ID/issues?assigneeAgentId=$AIDEVELO_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardAideveloPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  aideveloEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateAidevelo = parseObject(payloadTemplate.aidevelo);
  const workspace = asRecord(ctx.context.aideveloWorkspace);
  const workspaces = Array.isArray(ctx.context.aideveloWorkspaces)
    ? ctx.context.aideveloWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.aideveloRuntimeServiceIntents)
    ? ctx.context.aideveloRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardAidevelo: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: aideveloEnv.AIDEVELO_API_URL ?? null,
  };

  if (workspace) {
    standardAidevelo.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardAidevelo.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardAidevelo.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "aidevelo-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const aideveloEnv = buildAideveloEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, aideveloEnv);
  ...
  const aideveloPayload = buildStandardAideveloPayload(ctx, wakePayload, aideveloEnv, payloadTemplate);
