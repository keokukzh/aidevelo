...
import {
  ...
  AGENT_ROLES,
  AGENT_STATUSES,
  INTERNAL_AGENT_ADAPTER_TYPES,
} from "../constants.js";
import { CEO_BEHAVIOR_PROFILE } from "../ceo-behavior.js";
...
const ceoBehaviorSchema = z.object({
  profile: z.literal(CEO_BEHAVIOR_PROFILE).optional(),
  proactiveInitiativesEnabled: z.boolean().optional(),
  autonomousHiringEnabled: z.boolean().optional(),
});
...
const adapterConfigSchema = z.record(z.unknown()).superRefine((value, ctx) => {
  ...
  if (value.ceoBehavior !== undefined) {
    const ceoBehavior = ceoBehaviorSchema.safeParse(value.ceoBehavior);
    if (!ceoBehavior.success) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "adapterConfig.ceoBehavior must contain valid CEO behavior flags",
        path: ["ceoBehavior"],
      });
    }
  }
...
export const createAgentSchema = z.object({
  ...
  adapterType: z.enum(INTERNAL_AGENT_ADAPTER_TYPES).optional().default("process"),
