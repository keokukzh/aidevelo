...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://aidevelo:aidevelo@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "aidevelo");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://aidevelo:aidevelo@127.0.0.1:${preferredPort}/aidevelo`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://aidevelo:aidevelo@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "aidevelo");
    return {
      connectionString: `postgres://aidevelo:aidevelo@127.0.0.1:${port}/aidevelo`,
  ...
  const adminConnectionString = `postgres://aidevelo:aidevelo@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "aidevelo");

  return {
    connectionString: `postgres://aidevelo:aidevelo@127.0.0.1:${selectedPort}/aidevelo`,
