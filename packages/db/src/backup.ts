...
function resolveAideveloInstanceId(): string {
  const raw = process.env.AIDEVELO_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveAideveloHomeDir(), "instances", resolveAideveloInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveAideveloHomeDir(), "instances", resolveAideveloInstanceId(), "data", "backups");
}
