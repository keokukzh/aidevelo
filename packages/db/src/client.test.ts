...
async function createTempDatabase(): Promise<string> {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "aidevelo-db-client-"));
  ...
  const adminUrl = `postgres://aidevelo:aidevelo@127.0.0.1:${port}/postgres`;
  await ensurePostgresDatabase(adminUrl, "aidevelo");
