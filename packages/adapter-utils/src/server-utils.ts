...
const AIDEVELO_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface AideveloSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: AideveloSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "aidevelo_required",
      originLabel: "Required by Aidevelo",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Aidevelo",
    readOnly: false,
  };
}
...
export function buildAideveloEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    AIDEVELO_AGENT_ID: agent.id,
    AIDEVELO_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.AIDEVELO_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.AIDEVELO_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.AIDEVELO_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.AIDEVELO_API_URL = apiUrl;
  return vars;
}
...
export async function resolveAideveloSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...AIDEVELO_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listAideveloSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<AideveloSkillEntry[]> {
  const root = await resolveAideveloSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `aideveloai/aidevelo/${entry.name}`,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Aidevelo skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Aidevelo cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredAideveloRuntimeSkills(value: unknown): AideveloSkillEntry[] {
...
export async function readAideveloRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<AideveloSkillEntry[]> {
  const configuredEntries = normalizeConfiguredAideveloRuntimeSkills(config.aideveloRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listAideveloSkillEntries(moduleDir, additionalCandidates);
}

export async function readAideveloSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listAideveloSkillEntries(moduleDir);
...
export function readAideveloSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.aideveloSkillSync;
...
function canonicalizeDesiredAideveloSkillReference(
  reference: string,
...
export function resolveAideveloDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readAideveloSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredAideveloSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeAideveloSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.aideveloSkillSync;
  const current =
  ...
  next.aideveloSkillSync = current;
  return next;
}

export async function ensureAideveloSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Aidevelo server itself is started from
    // within a Claude Code session (e.g. `npx aideveloai run` in a terminal
