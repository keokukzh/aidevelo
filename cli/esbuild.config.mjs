/**
 * esbuild configuration for building the aideveloai CLI for npm.
 *
 * Bundles all workspace packages (@aideveloai/*) into a single file.
...
const workspacePaths = [
  ...
  "packages/adapters/aidevelo-gateway",
];

// Workspace packages that should NOT be bundled — they'll be published
// to npm and resolved at runtime (e.g. @aideveloai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@aideveloai/")) {
      externals.add(name);
    }
  }
