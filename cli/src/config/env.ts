...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Aidevelo CLI commands",
...
export function resolveAideveloEnvFile(configPath?: string): string {
...
export function loadAideveloEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeAideveloEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readAideveloEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeAideveloEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeAideveloEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readAideveloEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeAideveloEnvEntries(next, filePath);
  return next;
}
