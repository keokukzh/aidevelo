...
describe("AideveloApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new AideveloApiClient({
    ...
    expect(headers["x-aidevelo-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new AideveloApiClient({ apiBase: "http://localhost:3100" });
