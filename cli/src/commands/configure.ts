...
function defaultConfig(): AideveloConfig {
...
export async function configure(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" aidevelo configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `aideveloai onboard` first.");
    p.outro("");
    return;
  }

  let config: AideveloConfig;
  try {
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because AIDEVELO_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
