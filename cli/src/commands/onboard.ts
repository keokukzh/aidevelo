...
type OnboardDefaults = Pick<AideveloConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "AIDEVELO_PUBLIC_URL",
  "DATABASE_URL",
  "AIDEVELO_DB_BACKUP_ENABLED",
  "AIDEVELO_DB_BACKUP_INTERVAL_MINUTES",
  "AIDEVELO_DB_BACKUP_RETENTION_DAYS",
  "AIDEVELO_DB_BACKUP_DIR",
  "AIDEVELO_DEPLOYMENT_MODE",
  "AIDEVELO_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "AIDEVELO_ALLOWED_HOSTNAMES",
  "AIDEVELO_AUTH_BASE_URL_MODE",
  "AIDEVELO_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "AIDEVELO_STORAGE_PROVIDER",
  "AIDEVELO_STORAGE_LOCAL_DIR",
  "AIDEVELO_STORAGE_S3_BUCKET",
  "AIDEVELO_STORAGE_S3_REGION",
  "AIDEVELO_STORAGE_S3_ENDPOINT",
  "AIDEVELO_STORAGE_S3_PREFIX",
  "AIDEVELO_STORAGE_S3_FORCE_PATH_STYLE",
  "AIDEVELO_SECRETS_PROVIDER",
  "AIDEVELO_SECRETS_STRICT_MODE",
  "AIDEVELO_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveAideveloInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.AIDEVELO_PUBLIC_URL?.trim() ||
    process.env.AIDEVELO_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.AIDEVELO_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.AIDEVELO_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.AIDEVELO_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.AIDEVELO_ALLOWED_HOSTNAMES
    ? process.env.AIDEVELO_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.AIDEVELO_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.AIDEVELO_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.AIDEVELO_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.AIDEVELO_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.AIDEVELO_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.AIDEVELO_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.AIDEVELO_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.AIDEVELO_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.AIDEVELO_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.AIDEVELO_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.AIDEVELO_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.AIDEVELO_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.AIDEVELO_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.AIDEVELO_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.AIDEVELO_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "AIDEVELO_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<AideveloConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printAideveloCliBanner();
  p.intro(pc.bgCyan(pc.black(" aideveloai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveAideveloInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: AideveloConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@aideveloai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `aideveloai doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("AIDEVELO_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.AIDEVELO_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("AIDEVELO_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("AIDEVELO_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: AideveloConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: AIDEVELO_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("aideveloai run")}`,
      `Reconfigure later: ${pc.cyan("aideveloai configure")}`,
      `Diagnose setup: ${pc.cyan("aideveloai doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.AIDEVELO_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("aideveloai run")}`,
        `Then: ${pc.cyan("aideveloai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
