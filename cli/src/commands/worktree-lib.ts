...
export const DEFAULT_WORKTREE_HOME = "~/.aidevelo-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".aidevelo");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: AideveloConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "aidevelo",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    AIDEVELO_HOME: paths.homeDir,
    AIDEVELO_INSTANCE_ID: paths.instanceId,
    AIDEVELO_CONFIG: paths.configPath,
    AIDEVELO_CONTEXT: paths.contextPath,
    AIDEVELO_IN_WORKTREE: "true",
    ...(branding?.name ? { AIDEVELO_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { AIDEVELO_WORKTREE_COLOR: branding.color } : {}),
  };
}
