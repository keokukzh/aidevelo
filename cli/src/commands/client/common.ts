...
import { ApiRequestError, AideveloApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Aidevelo config file")
    .option("-d, --data-dir <path>", "Aidevelo data directory root (isolates state from ~/.aidevelo)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Aidevelo API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.AIDEVELO_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.AIDEVELO_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.AIDEVELO_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set AIDEVELO_COMPANY_ID, or set context profile companyId via `aideveloai context set`.",
    );
  }

  const api = new AideveloApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.AIDEVELO_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.AIDEVELO_SERVER_PORT || "");
