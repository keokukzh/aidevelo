...
async function collectPackageFiles(
  ...
  for (const entry of entries) {
    ...
    const isAideveloYaml = entry.name === ".aidevelo.yaml" || entry.name === ".aidevelo.yml";
    const contentType = binaryContentTypeByExtension[path.extname(entry.name).toLowerCase()];
    if (!isMarkdown && !isAideveloYaml && !contentType) continue;
          ...
          printOutput(
            {
              ...
              aideveloExtensionPath: exported.aideveloExtensionPath,
