...
function buildAgentEnvExports(input: {
  ...
  return [
    `export AIDEVELO_API_URL='${escaped(input.apiBase)}'`,
    `export AIDEVELO_COMPANY_ID='${escaped(input.companyId)}'`,
    `export AIDEVELO_AGENT_ID='${escaped(input.agentId)}'`,
    `export AIDEVELO_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Aidevelo skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Aidevelo skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveAideveloSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Aidevelo skills directory. Expected ./skills in the repo checkout.",
              );
            }
