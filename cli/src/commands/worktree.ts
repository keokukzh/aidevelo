...
import { ensureAgentJwtSecret, loadAideveloEnvFile, mergeAideveloEnvEntries, readAideveloEnvEntries, resolveAideveloEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { AideveloConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printAideveloCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.AIDEVELO_CONFIG;
...
const WORKTREE_NAME_PREFIX = "aidevelo-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.AIDEVELO_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.AIDEVELO_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromAideveloConfig(config: AideveloConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromAideveloConfig(config);
}
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.aidevelo"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: AideveloConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://aidevelo:aidevelo@127.0.0.1:${port}/aidevelo`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: AideveloConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.AIDEVELO_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.AIDEVELO_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.AIDEVELO_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.AIDEVELO_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: AideveloConfig;
  targetConfig: AideveloConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveAideveloEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readAideveloEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://aidevelo:aidevelo@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "aidevelo");
    const targetConnectionString = `postgres://aidevelo:aidevelo@127.0.0.1:${targetHandle.port}/aidevelo`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readAideveloEnvEntries(resolveAideveloEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.AIDEVELO_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.AIDEVELO_AGENT_JWT_SECRET);
  mergeAideveloEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { AIDEVELO_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadAideveloEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Aidevelo inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printAideveloCliBanner();
  p.intro(pc.bgCyan(pc.black(" aideveloai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printAideveloCliBanner();
  p.intro(pc.bgCyan(pc.black(" aideveloai worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasAideveloConfig: existsSync(path.resolve(worktreePath, ".aidevelo", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printAideveloCliBanner();
  p.intro(pc.bgCyan(pc.black(" aideveloai worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveAideveloEnvFile(configPath);
  const envEntries = readAideveloEnvEntries(envPath);
  const out = {
    AIDEVELO_CONFIG: configPath,
    ...(envEntries.AIDEVELO_HOME ? { AIDEVELO_HOME: envEntries.AIDEVELO_HOME } : {}),
    ...(envEntries.AIDEVELO_INSTANCE_ID ? { AIDEVELO_INSTANCE_ID: envEntries.AIDEVELO_INSTANCE_ID } : {}),
    ...(envEntries.AIDEVELO_CONTEXT ? { AIDEVELO_CONTEXT: envEntries.AIDEVELO_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasAideveloConfig)
      .map((choice) => path.resolve(choice.worktree, ".aidevelo", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readAideveloEnvEntries(resolveAideveloEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Aidevelo once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasAideveloConfig ? "aidevelo" : "no-aidevelo-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".aidevelo", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".aidevelo", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .aidevelo/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasAideveloConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Aidevelo worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasAideveloConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Aidevelo worktrees were found. Run `aideveloai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Aidevelo configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Aidevelo instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Aidevelo instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with aidevelo- if needed (created at ~/aidevelo-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: AIDEVELO_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: AIDEVELO_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source AIDEVELO_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Aidevelo instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Aidevelo worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with aidevelo- if needed")
