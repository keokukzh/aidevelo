...
const DEFAULT_AGENT_JWT_ISSUER = "aidevelo";
const DEFAULT_AGENT_JWT_AUDIENCE = "aidevelo-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveAideveloInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveAideveloInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" aidevelo env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: AideveloConfig | null = null;
...
function collectDeploymentEnvRows(config: AideveloConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrl =
    process.env.AIDEVELO_PUBLIC_URL ??
    process.env.AIDEVELO_AUTH_PUBLIC_BASE_URL ??
  ...
  const publicUrlSource: EnvSource =
    process.env.AIDEVELO_PUBLIC_URL
      ? "env"
      : process.env.AIDEVELO_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const secretsProvider =
    process.env.AIDEVELO_SECRETS_PROVIDER ??
    config?.secrets?.provider ??
    DEFAULT_SECRETS_PROVIDER;
  const secretsStrictMode =
    process.env.AIDEVELO_SECRETS_STRICT_MODE ??
    String(config?.secrets?.strictMode ?? false);
  const secretsKeyFilePath =
    process.env.AIDEVELO_SECRETS_MASTER_KEY_FILE ??
    config?.secrets?.localEncrypted?.keyFilePath ??
    defaultSecretsKeyFilePath();
  const storageProvider =
    process.env.AIDEVELO_STORAGE_PROVIDER ??
    config?.storage?.provider ??
    DEFAULT_STORAGE_PROVIDER;
  const storageLocalDir =
    process.env.AIDEVELO_STORAGE_LOCAL_DIR ??
    config?.storage?.localDisk?.baseDir ??
    defaultStorageBaseDir();
  const storageS3Bucket =
    process.env.AIDEVELO_STORAGE_S3_BUCKET ??
    config?.storage?.s3?.bucket ??
    "aidevelo";
  const storageS3Region =
    process.env.AIDEVELO_STORAGE_S3_REGION ??
    config?.storage?.s3?.region ??
    "us-east-1";
  const storageS3Endpoint =
    process.env.AIDEVELO_STORAGE_S3_ENDPOINT ??
    config?.storage?.s3?.endpoint ??
    "";
  const storageS3Prefix =
    process.env.AIDEVELO_STORAGE_S3_PREFIX ??
    config?.storage?.s3?.prefix ??
    "";
  const storageS3ForcePathStyle =
    process.env.AIDEVELO_STORAGE_S3_FORCE_PATH_STYLE ??
    String(config?.storage?.s3?.forcePathStyle ?? false);

  const rows: EnvVarRow[] = [
    {
      key: "AIDEVELO_AGENT_JWT_SECRET",
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from AIDEVELO_PUBLIC_URL when possible)",
    },
    {
      key: "AIDEVELO_AGENT_JWT_TTL_SECONDS",
      value: process.env.AIDEVELO_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.AIDEVELO_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "AIDEVELO_AGENT_JWT_ISSUER",
      value: process.env.AIDEVELO_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.AIDEVELO_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "AIDEVELO_AGENT_JWT_AUDIENCE",
      value: process.env.AIDEVELO_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.AIDEVELO_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "AIDEVELO_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.AIDEVELO_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "AIDEVELO_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.AIDEVELO_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "AIDEVELO_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.AIDEVELO_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "AIDEVELO_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.AIDEVELO_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "AIDEVELO_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.AIDEVELO_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "AIDEVELO_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.AIDEVELO_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "AIDEVELO_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.AIDEVELO_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "AIDEVELO_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.AIDEVELO_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "AIDEVELO_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.AIDEVELO_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "AIDEVELO_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.AIDEVELO_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.AIDEVELO_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "AIDEVELO_CONFIG",
      value: process.env.AIDEVELO_CONFIG ?? configPath,
      source: process.env.AIDEVELO_CONFIG ? "env" : "default",
      required: false,
