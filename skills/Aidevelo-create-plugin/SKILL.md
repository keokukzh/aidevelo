---
name: aidevelo-create-plugin
description: >
  Create new Aidevelo plugins with the current alpha SDK/runtime. Use when
...
# Create a Aidevelo Plugin

Use this skill when the task is to create, scaffold, or document a Aidevelo plugin.
...
node packages/plugins/create-aidevelo-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Aidevelo repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.aidevelo-sdk/`:

```bash
pnpm --filter @aideveloai/create-aidevelo-plugin build
node packages/plugins/create-aidevelo-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Aidevelo from an absolute local path during development
