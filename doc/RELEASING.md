...
Maintainer runbook for shipping Aidevelo across npm, GitHub, and the website-facing changelog surface.
...
Aidevelo uses calendar versions that still fit semver syntax:
...
2. **npm** — `aideveloai` and public workspace packages are published
...
```bash
npx aideveloai@canary onboard
# or
npx aideveloai@canary onboard --data-dir "$(mktemp -d /tmp/aidevelo-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/aideveloai/aidevelo/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Aidevelo. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
AIDEVELOAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
AIDEVELOAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary AIDEVELOAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable AIDEVELOAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f aidevelo_version=canary
gh workflow run release-smoke.yml -f aidevelo_version=latest
```

Minimum checks:

- `npx aideveloai@canary onboard` installs
