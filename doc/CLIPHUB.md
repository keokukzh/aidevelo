...
ClipHub is the public registry where people share, discover, and download Aidevelo company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Aidevelo what a package registry is to a programming language. Aidevelo already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Aidevelo — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Aidevelo instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Aidevelo spec. It contains:
...
**Install (fresh start):**
```
aidevelo install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Aidevelo instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Aidevelo, export your company as a template, then publish:

```
aidevelo export --template my-company
aidevelo publish cliphub my-company
```
...
ClipHub is a **separate service** from Aidevelo itself. Aidevelo is self-hosted; ClipHub is a hosted registry that Aidevelo instances talk to.
...
| **Aidevelo CLI** | `aideveloai install`, `aideveloai publish`, `aideveloai cliphub sync` — built into Aidevelo |
| **Aidevelo UI** | "Browse ClipHub" panel in the Aidevelo web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Aidevelo) |
| Backend | TypeScript + Hono (consistent with Aidevelo) |
| Database | PostgreSQL |
...
4. Run `aideveloai install cliphub:acme/lean-saas-shop`
5. Aidevelo creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Aidevelo until it works well
2. Export: `aideveloai export --template my-agency`
3. Publish: `aideveloai publish cliphub my-agency`
...
3. Install just that agent: `aideveloai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Aidevelo

ClipHub is **not required** to use Aidevelo. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Aidevelo what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `aideveloai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Aidevelo web UI
- [ ] `aideveloai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Aidevelo's own philosophy)
