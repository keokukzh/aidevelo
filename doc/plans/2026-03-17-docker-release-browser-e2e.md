...
Today release smoke testing for published Aidevelo packages is manual and shell-driven:
...
- Docker
- `npx aideveloai@canary`
- `npx aideveloai@latest`
...
- runs `aideveloai@${AIDEVELOAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@aidevelo.local`
- password: `aidevelo-smoke-password`
...
- the suite passes locally against both:
  - `AIDEVELOAI_VERSION=canary`
  - `AIDEVELOAI_VERSION=latest`
...
1. A published `aideveloai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `aideveloai@latest` Docker install can be smoke-tested by Playwright in CI.
