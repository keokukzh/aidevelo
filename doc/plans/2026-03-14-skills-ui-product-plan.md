...
This document defines the product and UI plan for skill management in Aidevelo.
...
- Aidevelo company import/export can include skills as package content
...
3. The UI must always tell the truth about what Aidevelo knows:
   - desired state in Aidevelo
...
Aidevelo should treat skills at two scopes:
...
- created locally inside Aidevelo later
...
7. “Understand what Aidevelo knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Aidevelo-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Aidevelo did not install.

These should be clearly marked:

- external
- not managed by Aidevelo
...
- `desired_only`: Aidevelo wants it, adapter does not show it yet
- `external`: adapter has it but Aidevelo does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Aidevelo only tracks desired state
...
- Aidevelo can still track desired skills
...
- Aidevelo can see adapter skills
...
- `.aidevelo.yaml` may add Aidevelo-specific fidelity, but should not replace the base shortname association model
...
- `.aidevelo.yaml` may add Aidevelo-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Aidevelo should have a company-scoped skill model or managed package model representing:
...
Aidevelo should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Aidevelo-specific.
...
That gives Aidevelo one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
