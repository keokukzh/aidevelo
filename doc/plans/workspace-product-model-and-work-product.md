...
Aidevelo needs to support two very different but equally valid ways of working:
...
Today, Aidevelo already has the beginnings of this model:
...
- making Aidevelo feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Aidevelo users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Aidevelo deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Aidevelo into a full code review product
...
This object must be recorded explicitly so that Aidevelo can:
...
Aidevelo should treat PRs as a type of work product linked back to:
...
Aidevelo already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Aidevelo must be a first-class deployment mode

Aidevelo cannot assume that it is running on the same machine as the code.

In cloud deployments, Aidevelo may:
...
- `Runtime service`: a process or service Aidevelo owns or tracks for a workspace
...
It also lets cloud-hosted Aidevelo deployments point at codebases and remotes without pretending the Aidevelo host has direct filesystem access.
...
Without an explicit `execution workspace` record, Aidevelo has nowhere stable to attach:
...
Aidevelo needs a single place to show:
...
- Aidevelo-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Aidevelo owning the execution host
...
## 1. Aidevelo host and execution host must be decoupled
...
without the Aidevelo host itself running local git or local preview processes.
...
Aidevelo should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Aidevelo.
...
Aidevelo should keep workspace automation centered on a small set of hooks:
...
Aidevelo should provide the equivalent visibility in the project `Code` surface:
...
Aidevelo should treat execution workspaces as the natural home for:
...
Aidevelo should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Aidevelo should continue treating cleanup policy as part of the core workspace model:
...
Aidevelo should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Aidevelo host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Aidevelo should borrow Worktrunk's good execution patterns while keeping the broader Aidevelo model:
...
That keeps Aidevelo focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Aidevelo deployments remain first-class

That is a better fit for Aidevelo than either extreme:
