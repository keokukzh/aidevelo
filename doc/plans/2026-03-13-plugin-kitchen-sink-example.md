...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Aidevelo plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Aidevelo.
...
- package: `@aideveloai/plugin-kitchen-sink-example`
- plugin id: `aidevelo.kitchen-sink-example` or `aidevelo-kitchen-sink-example`

Recommendation: use `aidevelo-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Aidevelo event such as `issue.created`
...
Recommendation: default to a Aidevelo-local endpoint or a stable public echo endpoint to avoid flaky docs.
