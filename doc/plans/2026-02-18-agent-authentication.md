...
Agents need API keys to authenticate with Aidevelo. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Aidevelo
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Aidevelo generates a token and passes it directly to the agent
...
`AIDEVELO_API_URL`, `AIDEVELO_AGENT_ID`, `AIDEVELO_COMPANY_ID`. We need to
add a `AIDEVELO_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Aidevelo CLI
...
```
aidevelo auth login
# Opens browser -> user confirms -> token stored at ~/.aidevelo/credentials
```
...
managed by the Aidevelo server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Aidevelo.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Aidevelo SKILL.md (or a link to it)
   - What information Aidevelo needs from the agent (e.g. webhook URL, adapter
...
5. Aidevelo stores the pending registration.
...
7. On approval, Aidevelo provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Aidevelo issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.aidevelo.ing/skills/aidevelo/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Aidevelo should send you heartbeats",
...
1. Generate a Aidevelo invite link for the company.
...
4. A Aidevelo company member approves the new agent.
5. Aidevelo begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `AIDEVELO_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `aideveloai auth login` for developer-managed remote agents.                                      |
