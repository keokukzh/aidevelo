...
`PAP-447` asks how Aidevelo should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Aidevelo instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Aidevelo:

- some operators use Aidevelo to manage Aidevelo and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Aidevelo
- Claude and Codex expose different built-in affordances, so Aidevelo should not overfit to one tool

## Core Product Decision

Aidevelo should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Aidevelo
...
- Aidevelo should not assume that every adapter can see or use a host filesystem path directly
...
If Aidevelo makes worktrees agent-first, it will blur:
...
- Aidevelo core resolves and records execution workspace state
...
That gives Aidevelo room to support:
...
- Aidevelo handles workspace runtime services under the hood using project/workspace policy
...
Once Aidevelo is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Aidevelo should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Aidevelo cannot infer
...
- when issue moves to done, should Aidevelo auto-commit?
- should it auto-push?
...
Aidevelo already has the right foundation for a project-first model.
...
For server-spawned local adapters, Aidevelo already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Aidevelo should mirror directly

Implication:

- for `codex_local`, Aidevelo should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Aidevelo should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Aidevelo should separate:
...
Aidevelo should be able to express intentions such as:
...
These are not specific to Aidevelo. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".aidevelo/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Aidevelo-specific `devServer` flag.
...
Aidevelo should distinguish between:
...
Aidevelo should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Aidevelo should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Aidevelo
...
For runtime services specifically, Aidevelo should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Aidevelo-only dev-server flags
...
3. Inject resolved execution workspace details into `context.aideveloWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Aidevelo instance can be reused through env/config injection when appropriate
...
- Aidevelo can decide whether to reuse or start a fresh service deterministically
...
3. Test against Aidevelo-on-Aidevelo workflows first.
...
7. Workspace runtime services are modeled generically, not as Aidevelo-specific dev-server toggles.
