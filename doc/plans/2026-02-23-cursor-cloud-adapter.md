...
This document defines the V1 design for a Aidevelo adapter that integrates with
...
4. synthesized stdout events for Aidevelo UI/CLI
...
3. **Agent callback auth to Aidevelo** uses a bootstrap exchange flow (no long-lived Aidevelo key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Aidevelo endpoints, not full SKILL.md prompt injection.
...
- `aideveloPublicUrl` (optional override; else `AIDEVELO_PUBLIC_URL` env)
...
## Aidevelo Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `AIDEVELO_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.aideveloPublicUrl`
2. `process.env.AIDEVELO_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Aidevelo credentials in prompt text.

Flow:

1. Before launch/follow-up, Aidevelo mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Aidevelo validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Aidevelo API calls.
...
1. Prompt includes a compact instruction to fetch skills from Aidevelo.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/aidevelo`
   - `GET /api/skills/aidevelo-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <aideveloPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Aidevelo run context.
...
5. `aideveloPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Aidevelo skill endpoints |
| Aidevelo auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Aidevelo context.
3. Add richer PR metadata surfacing in Aidevelo UI.
