...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/aidevelo-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `AIDEVELO_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `AIDEVELO_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Aidevelo may create host-local worktrees, processes, and paths
- `cloud`
  - Aidevelo should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `aidevelo | github | vercel | s3 | custom`
...
- use `AIDEVELO_EXECUTION_TOPOLOGY` as a defaulting guardrail
