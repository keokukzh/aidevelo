...
This document defines the next-stage plan for Aidevelo company import/export.

The core shift is:

- move from a Aidevelo-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Aidevelo.
...
Aidevelo already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Aidevelo
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `aidevelo.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.aidevelo.yaml` sidecar for high-fidelity Aidevelo-specific details
...
11. adapter-aware skill sync surfaces so Aidevelo can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Aidevelo at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Aidevelo sidecar.
...
Aidevelo must not redefine `SKILL.md`.
...
- Aidevelo-specific fidelity lives in `.aidevelo.yaml`
- Aidevelo may resolve and install `SKILL.md` packages, but it must not require a Aidevelo-only skill format
...
### 5.4 Base Package Vs Aidevelo Extension
...
- base package:
  ...
  - no Aidevelo-only runtime fields by default
- Aidevelo extension:
  - `.aidevelo.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Aidevelo tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`aidevelo.manifest.json` is not part of the future package direction.
...
Aidevelo import/export should support these entity kinds:
...
- Aidevelo can treat “this set of agents came from team package X” as the imported-team model
...
Aidevelo should support source references in package metadata with:
...
Aidevelo should define a new adapter surface area around skills:
...
Planned Aidevelo behavior:

- if an adapter supports read, Aidevelo should show current skills in the UI
- if an adapter supports write, Aidevelo should let the user enable/disable imported skills
- if an adapter supports sync, Aidevelo should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.aidevelo.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.aidevelo.yaml` alongside the base package
...
## 10. Storage Model Inside Aidevelo
...
Aidevelo should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `aideveloai company export <company-id> --out <path>`
- `aideveloai company import --from <path-or-url> --dry-run`
- `aideveloai company import --from <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Aidevelo storage, or only referenced at import time?
...
3. Should Aidevelo support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Aidevelo aligned with:
