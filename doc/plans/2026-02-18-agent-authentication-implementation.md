...
- New config/env settings:
  - `AIDEVELO_AGENT_JWT_SECRET`
  - `AIDEVELO_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `AIDEVELO_AGENT_JWT_ISSUER` (default: `aidevelo`)
  - `AIDEVELO_AGENT_JWT_AUDIENCE` (default: `aidevelo-api`)
...
1. In:
   ...
   inject `AIDEVELO_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `AIDEVELO_API_KEY`, do not overwrite it.
- Continue injecting:
  - `AIDEVELO_AGENT_ID`
  - `AIDEVELO_COMPANY_ID`
  - `AIDEVELO_API_URL`
...
- Local adapters authenticate without manual `AIDEVELO_API_KEY` config.
