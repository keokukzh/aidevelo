...
Aidevelo's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Aidevelo's API, UI, and data model | Yes |
...
modules/
  observability/
    aidevelo.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@aideveloai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@aideveloai/core";
...
```
1. Scan modules/ directory for aidevelo.module.json manifests
...
```jsonc
// aidevelo.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@aideveloai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@aideveloai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:aidevelo/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.aidevelo.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm aideveloai store list                    # browse available modules and templates
pnpm aideveloai store install <module-id>     # install a module
pnpm aideveloai store import <template-id>    # import a company template
pnpm aideveloai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Aidevelo instance | `server:started` |
...
4. **Module config** — `aidevelo.config.json` with per-module config, env var interpolation
...
10. **`@aideveloai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
