...
This document defines the rollout plan for adapter-wide skill support in Aidevelo.
...
- Aidevelo stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Aidevelo currently has these adapters:
...
1. Desired skills live in Aidevelo for every adapter.
...
These adapters have a stable local skills directory that Aidevelo can read and manage.
...
These adapters do not have a meaningful Aidevelo-owned persistent install state.
...
- desired skills stored in Aidevelo
...
- runtime already injects Aidevelo skills into `~/.cursor/skills`
...
- runtime already injects Aidevelo skills into `~/.gemini/skills`
...
- runtime already injects Aidevelo skills into `~/.pi/agent/skills`
...
- Aidevelo can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Aidevelo skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Aidevelo may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Aidevelo skills that are clearly marked as Aidevelo-managed
...
- Aidevelo stores desired skills only
...
- avoid destructive wording unless Aidevelo can prove a skill is Aidevelo-managed
...
That gets Aidevelo from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
