...
Add a single storage subsystem for Aidevelo that supports:
...
- API serves attachment bytes through authenticated Aidevelo endpoints.
...
- Default local path is under instance root: `~/.aidevelo/instances/<instanceId>/data/storage`.
...
- `aideveloai onboard` writes a valid `storage` config block by default.
- `aideveloai configure --section storage` can switch between local and s3 modes.
...
- `aideveloai doctor` reports actionable storage status.
