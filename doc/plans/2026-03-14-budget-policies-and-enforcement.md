...
Aidevelo already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Aidevelo should implement next.

## Product Goals

Aidevelo should let operators:
...
For the next implementation pass, Aidevelo should enforce these defaults:
...
Aidevelo should separate subscription-included usage from billed spend:
...
Aidevelo should have two threshold classes:
...
Aidevelo must know that a project is budget-paused and enforce it.
...
Aidevelo must also block execution before new work starts.
