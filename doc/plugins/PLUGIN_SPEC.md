# Aidevelo Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Aidevelo's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Aidevelo app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Aidevelo HTTP APIs directly.
...
Aidevelo plugin design is based on the following assumptions:

1. Aidevelo is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Aidevelo business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Aidevelo core.
...
2. Let plugins add major capabilities without editing Aidevelo core.
...
4. Depend on project-local plugin folders such as `.aidevelo/plugins`.
...
The single Aidevelo deployment an operator installs and controls.

### 5.2 Company

A first-class Aidevelo business object inside the instance.
...
A trusted in-process extension loaded directly by Aidevelo core.
...
An installable instance-wide extension package loaded through the Aidevelo plugin runtime.
...
Aidevelo has two extension classes.
...
Aidevelo already has a concrete workspace model:
...
Plugins live under the Aidevelo instance directory.

Suggested layout:

- `~/.aidevelo/instances/default/plugins/package.json`
- `~/.aidevelo/instances/default/plugins/node_modules/`
- `~/.aidevelo/instances/default/plugins/.cache/`
- `~/.aidevelo/instances/default/data/plugins/<plugin-id>/`
...
Aidevelo should add CLI commands:

- `pnpm aideveloai plugin list`
- `pnpm aideveloai plugin install <package[@version]>`
- `pnpm aideveloai plugin uninstall <plugin-id>`
- `pnpm aideveloai plugin upgrade <plugin-id> [version]`
- `pnpm aideveloai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@aidevelo/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface AideveloPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Aidevelo agents can use during runs.
...
- Aidevelo server starts one worker process per installed plugin
...
Receives one typed Aidevelo domain event.
...
export function definePlugin(definition: PluginDefinition): AideveloPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: AideveloPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@aideveloai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@aideveloai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@aideveloai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@aideveloai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@aideveloai/plugin-sdk/ui`
...
The `@aideveloai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Aidevelo secret provider system rather than a plain text input
...
1. Core Aidevelo data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Aidevelo objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Aidevelo product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Aidevelo later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Aidevelo secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm aideveloai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Aidevelo server. This is a normative requirement, not optional.
...
### 27.1 `@aideveloai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@aideveloai/plugin-test-harness";
...
For developing a plugin against a running Aidevelo instance:

- The operator installs the plugin from a local path: `pnpm aideveloai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-aidevelo-plugin`) that scaffolds:

- `package.json` with correct `aideveloPlugin` keys
...
- `@aidevelo/plugin-workspace-files`
- `@aidevelo/plugin-terminal`
- `@aidevelo/plugin-git`
- `@aidevelo/plugin-linear`
- `@aidevelo/plugin-github-issues`
- `@aidevelo/plugin-grafana`
- `@aidevelo/plugin-runtime-processes`
- `@aidevelo/plugin-stripe`
...
- `@aideveloai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@aideveloai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@aideveloai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@aideveloai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@aideveloai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@aideveloai/plugin-sdk/ui`
...
- `@aideveloai/plugin-test-harness`
- `create-aidevelo-plugin` starter template
...
Aidevelo should not implement a generic in-process hook bag modeled directly after local coding tools.

Aidevelo should implement:
...
That is the complete target design for the Aidevelo plugin system.
