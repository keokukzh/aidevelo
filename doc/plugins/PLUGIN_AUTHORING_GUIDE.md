...
This guide describes the current, implemented way to create a Aidevelo plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Aidevelo app.
...
```bash
pnpm --filter @aideveloai/create-aidevelo-plugin build
node packages/plugins/create-aidevelo-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Aidevelo repo:

```bash
pnpm --filter @aideveloai/create-aidevelo-plugin build
node packages/plugins/create-aidevelo-plugin/dist/index.js @yourscope/plugin-name \
  --output /absolute/path/to/plugin-repos \
  --sdk-path /absolute/path/to/aidevelo/packages/plugins/sdk
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@aideveloai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@aideveloai/plugin-sdk` from the local Aidevelo checkout into a `.aidevelo-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Aidevelo from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@aideveloai/plugin-sdk/ui`
