...
This document summarizes the memory systems referenced in task `PAP-530` and extracts the design patterns that matter for Aidevelo.

## What Aidevelo Needs From This Survey

Aidevelo is not trying to become a single opinionated memory engine. The more useful target is a control-plane memory surface that:

- stays company-scoped
- lets each company choose a default memory provider
- lets specific agents override that default
- keeps provenance back to Aidevelo runs, issues, comments, and documents
...
The question is not "which memory project wins?" The question is "what is the smallest Aidevelo contract that can sit above several very different memory systems without flattening away the useful differences?"
...
These emphasize local persistence, inspectability, and low operational overhead. They are useful because Aidevelo is local-first today and needs at least one zero-config path.

## Per-Project Notes

| Project | Shape | Notable API / model | Strong fit for Aidevelo | Main mismatch |
|---|---|---|---|---|
...
| [mem0](https://github.com/mem0ai/mem0) | hosted + OSS SDK | `add`, `search`, `getAll`, `get`, `update`, `delete`, `deleteAll`; entity partitioning via `user_id`, `agent_id`, `run_id`, `app_id` | closest to a clean provider API with identities and metadata filters | provider owns extraction heavily; Aidevelo should not assume every backend behaves like mem0 |
| [MemOS](https://github.com/MemTensor/MemOS) | memory OS / framework | unified add-retrieve-edit-delete, memory cubes, multimodal memory, tool memory, async scheduler, feedback/correction | strong source for optional capabilities beyond plain search | much broader than the minimal contract Aidevelo should standardize first |
...
| [memU](https://github.com/NevaMind-AI/memU) | proactive agent memory framework | file-system metaphor, proactive loop, intent prediction, always-on companion model | good source for when memory should trigger agent behavior, not just retrieval | proactive assistant framing is broader than Aidevelo's task-centric control plane |
| [Memori](https://github.com/MemoriLabs/Memori) | hosted memory fabric + SDK wrappers | registers against LLM SDKs, attribution via `entity_id` + `process_id`, sessions, cloud + BYODB | strong example of automatic capture around model clients | wrapper-centric design does not map 1:1 to Aidevelo's run / issue / comment lifecycle |
...
If Aidevelo does not expose these, it will not adapt well to the systems above.

## Where The Systems Differ

These differences are exactly why Aidevelo needs a layered contract instead of a single hard-coded engine.
...
Aidevelo should support both:

- provider-managed extraction
- Aidevelo-managed extraction with provider-managed storage / retrieval
...
Aidevelo should not require a single storage shape. It should require normalized references back to Aidevelo entities.
...
Aidevelo should make plain search the minimum contract and richer outputs optional capabilities.
...
Aidevelo needs both direct request/response operations and background maintenance hooks.

## Aidevelo-Specific Takeaways

### Aidevelo should own these concerns

- binding a provider to a company and optionally overriding it per agent
- mapping Aidevelo entities into provider scopes
- provenance back to issue comments, documents, runs, and activity
- cost / token / latency reporting for memory work
- browse and inspect surfaces in the Aidevelo UI
...
Aidevelo does not need to standardize every feature from every provider. It needs:
...
Aidevelo should adopt a two-layer memory model:

1. `Memory binding + control plane layer`
   Aidevelo decides which provider key is in effect for a company, agent, or project, and it logs every memory operation with provenance and usage.

2. `Provider adapter layer`
   A built-in or plugin-supplied adapter turns Aidevelo memory requests into provider-specific calls.
...
without forcing Aidevelo itself to become a monolithic memory engine.
