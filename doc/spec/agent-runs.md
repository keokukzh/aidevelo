...
This spec defines how Aidevelo actually runs agents while staying runtime-agnostic.
...
1. Aidevelo is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Aidevelo manages lifecycle, and on exit Aidevelo parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `aidevelo.skill` (shared Aidevelo skill text block)
...
2. Preferred transport is env vars (`AIDEVELO_*`) injected at runtime.
